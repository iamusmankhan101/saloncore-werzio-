...
export async function GET(request: NextRequest) {
  ...
  try {
    const booking = await db.execute({
      sql: `SELECT id, kind, phone, client_name, appt_date, appt_time, scheduled_at, status, attempts, last_error, created_at
    ...
    const pos = await db.execute({
      sql: `SELECT id, invoice_id, invoice_number, phone, client_name, scheduled_at, status, attempts, last_error, created_at
