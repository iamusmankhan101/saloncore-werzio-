...
function AddEditServiceModal({ onClose, onSave, staffList, serviceToEdit }: { onClose: () => void; onSave: (s: Service) => void; staffList: Staff[]; serviceToEdit?: Service }) {
  ...
  const toggleStaff = (id: string) => {
    ...
    if (current.includes(id)) {
      set("assignedStaffIds", current.filter(x => x !== id));
    } else {
      set("assignedStaffIds", [...current, id]);
    }
  };
  ...
  if (done) return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 360, padding: "48px 32px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ fontSize: 13, color: "#9898b0", marginBottom: 24 }}>{isEditing ? "The service has been updated successfully." : "The new service has been saved successfully."}</div>
    ...
    <div style={{ background: "#f4f5f7", minHeight: "100vh", padding: "28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#9898b0", marginTop: 2 }}>{services.length} salon services available</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
        {[
          ...
          { label: "Premium Service Max", value: fmt(maxPrice), icon: <DollarSign size={18} color="#db2777" />, bg: "#fdf2f8" },
        ].map((stat, i) => (
      ...
      <div style={{ display: "flex", gap: 8, borderBottom: "1px solid #ebebf0", paddingBottom: 4 }}>
        {["all", "hair", "skin", "nails", "bridal"].map((cat) => {
          ...
          const label = cat === "all" ? "All Services" : CATEGORY_LABELS[cat]?.label;
          return (
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <button
                    ...
                    style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid #EDE9FE", background: "#F5F3FF", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.2s" }}
